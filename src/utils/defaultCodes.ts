...
export const getDefaultRandomCodes = (
  languageId: string
): { selectedCodes: string[]; testCases: TestCase[] } => {
  const languageConstants = constantCodes.find((c) => c.languageId === languageId);
  
  if (!languageConstants || languageConstants.codes.length === 0) {
    return { selectedCodes: [], testCases: [] };
  }

  const randomIndex = Math.floor(Math.random() * languageConstants.codes.length);
  const randomCode = languageConstants.codes[randomIndex];

  return {
    selectedCodes: [randomCode.codeA, randomCode.codeB],
    testCases: randomCode.testCases,
  };
};